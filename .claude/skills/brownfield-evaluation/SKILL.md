...
1. **Codebase health** — cyclomatic complexity, duplication, dead code, lint config, naming, comment-to-noise ratio. Quick-scan red flags: `// TODO` in production, commented-out code blocks, empty catch blocks, hardcoded URLs/credentials, magic strings/numbers. Systemic anti-patterns to probe for: parameter sprawl (5+ positional args signals missing abstractions), leaky abstractions (service layer returning ORM objects, components receiving raw API shapes), copy-paste variants (≥2 near-identical functions differing only by a variable or URL), redundant state (fields derivable from other fields — sync bugs waiting to happen), TOCTOU patterns (`if exists → operate` without atomic operation or lock), no-op updates (unconditional polling/interval writes to DB regardless of change).
...
6. **Security** — attack surface, input validation, secrets management, OWASP Top 10 exposure, audit logging. When code is available, systematically probe: **auth/authZ** (IDOR paths, RBAC gaps, session tokens from `Math.random()` not `crypto.randomBytes`, missing account lockout, JWT with weak secret or no expiry); **input validation** (SQL built via string concat, XSS via `innerHTML`/`dangerouslySetInnerHTML`, path traversal via unsanitized `req.params.filename`, command injection via `os.system(f"... {input}")`); **cryptography** (MD5/SHA1 for passwords instead of bcrypt/argon2, custom crypto implementations); **API surface** (CORS `origin: '*'`, missing per-IP/per-user rate limits, missing security headers); **data exposure** (PII or stack traces in log output, secrets in error API responses, tokens delivered in URL query params rather than httpOnly cookies).
7. **Data layer** — schema quality, N+1 risk, migration hygiene, connection pooling, schema drift vs models. Additional SQL-specific systemic probes: naive/server-local timestamps (`NOW()` without timezone awareness — DST bugs); `= NULL` comparisons instead of `IS NULL`; unguarded `SELECT *` in ORM or raw queries (schema drift amplifier); missing `LIMIT` on large-table reads; collation/case-sensitivity assumptions (MySQL and Postgres defaults differ — silent bugs on cross-DB migration); missing transactions around multi-step writes; missing indexes on columns used in frequent filters or joins.
...
- **Static analysis to run** — linters, complexity, dead-code, dependency-graph tools for their language. Also run dependency vulnerability scanning — fast, automated, and often surfaces CRITICAL/HIGH findings in minutes: `npm audit` (Node), `pip-audit` or `safety check` (Python), `cargo audit` (Rust), `snyk test` (any stack). Lock files must be committed (`package-lock.json`, `Cargo.lock`) for these to be accurate.
